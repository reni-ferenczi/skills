...
This component is the concrete leaves of the feature model: every scalar, geometry wrapper, rotation, time value, topological definition and raster that a GPML/GML feature can carry is a `PropertyValue` subclass defined here. The GPGIM types a feature by pairing a property name with a structural type, and it is this component that supplies both ends of that pairing — a `StructuralType` (an interned, qualified XML name) on every class, so the GPML/GPGIM readers and writers and `Gpgim` itself know which XML element a class represents and which C++ class to build for a given element. Above `property-values`, nothing else defines what a reconstructable feature is actually made of; below it, the classes here draw on `model`'s revisioning and interning machinery and on `maths`'s spherical geometry, but add no reconstruction behaviour of their own — reconstruction, layers and rendering are all built on top of the values this component stores.

`GeoTimeInstant` is the load-bearing type by a wide margin: the scalar time value with two open ends (distant past, distant future) that every reconstruction time, feature validity range and animation frame passes around directly, deliberately kept outside the `PropertyValue` hierarchy and wrapped into it by `GmlTimeInstant`/`GmlTimePeriod`. `RawRaster`, `RawRasterUtils` and `RasterType` form the raster family's core: a closed, policy-based template of raster storage strategies with no runtime polymorphism, made usable through a shared visitor layer and a runtime type tag that bridges it to GDAL and the rest of the codebase; `Georeferencing` sits beside them as the one neutral representation of where a raster sits on the globe, handed from `GmlRectifiedGrid` through the file-io readers/writers to the OpenGL rendering path. `StructuralType` and `ValueObjectType` are the interned qualified-name typedefs that tag, respectively, every property value's own type and the named scalars inside a `GmlDataBlockCoordinateList`. `GpmlTimeSample`, paired with `GpmlIrregularSampling`, is the backbone of total reconstruction sequences and finite-rotation interpolation; `GpmlKeyValueDictionary` is how arbitrary attribute tables — most visibly shapefile/OGR attribute columns — attach to a feature since GPML has no native schema for them; `GpmlAge` and `GmlPoint` both trade a single canonical representation for deliberately preserving whichever form (absolute or named age; spherical or 2D point) the data arrived in; `TextContent` is the interned string that backs `XsString` and other free-text content; `Enumeration` lets one C++ class serve every distinct GPML enumeration type by storing type and value as data rather than as a subclass per type.

The component leans hardest on `model`, for the `PropertyValue`/`GpmlConstantValue`-style revisioning base and the `StringSet`/`QualifiedXmlName` interning machinery every typedef here instantiates, and on `maths`, whose `PointOnSphere`, `PolylineOnSphere`, `PolygonOnSphere` and `MultiPointOnSphere` are what the `Gml*` geometry classes wrap; smaller dependencies reach into `file-io` for the `RasterBandReaderHandle` behind proxied rasters and into `gui` for the `rgba8_t` type that gives `RasterType` its RGBA8 element. The relationship with `model` also runs the other way — `Gpgim` and `ModelUtils` construct default and example property values and structural-type definitions directly off these classes, so the GPGIM schema itself is partly expressed in terms of `property-values` types. Everything downstream leans on this component far more than it leans out: `file-io` is overwhelmingly the heaviest consumer, since every reader and writer constructs and serialises these classes from and to GPML/GML/shapefile/raster formats; `qt-widgets` builds its property-editing widgets (ages, time samples, rotations, key-value dictionaries, affine georeferencing) directly against these types; `app-logic` drives reconstruction against `GeoTimeInstant`, `GpmlFiniteRotation` and the topological section types; and `gui` and `opengl` consume `RawRaster`, `RasterType`, `Georeferencing` and `RasterStatistics` to turn proxied raster data into rendered globe and map imagery.
