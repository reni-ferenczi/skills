...
`GPlatesPresentation::TopologyNetworkVisualLayerParams` is the `VisualLayerParams` for a topological-network layer, and is considerably richer than the geometry/velocity counterparts because a network can be drawn several different ways: as a triangulation mesh, filled by draw style, or coloured by one of three derived strain-rate quantities (dilatation, second invariant, "strain rate style"). `TriangulationDrawMode` selects the drawing style and `TriangulationColourMode` selects which of those quantities (if any) drives the colouring; each colourable quantity gets its own min/max range and its own `GPlatesGui::ColourPalette<double>`, generated by a `create_default_*_colour_palette()` method or loaded from a CPT file via `set_*_colour_palette()`.

The layer also independently controls whether rigid blocks embedded in the network and the triangulation itself are filled, with shared opacity/intensity/modulate-colour settings, and whether segment velocities are drawn. The free `transcribe()` overloads for `TriangulationColourMode` and `TriangulationDrawMode` let the Scribe system persist these enums by name in sessions and projects (see `TranscribeSession`), rather than by their numeric value.
...
- The three strain-rate colour palettes (`d_dilatation_colour_palette`, `d_second_invariant_colour_palette`, `d_strain_rate_style_colour_palette`) are `boost::optional` and only populated once a default has been generated or a file loaded; an empty filename field (`d_*_colour_palette_filename`) means the auto-generated default is in use rather than one loaded from a CPT file, matching the convention used by `RemappedColourPaletteParameters`.
- `TriangulationColourMode`/`TriangulationDrawMode` must go through the dedicated `transcribe()` overloads (rather than being transcribed as raw enum values) so that saved sessions and projects stay readable across GPlates versions that add or reorder enumerators.
